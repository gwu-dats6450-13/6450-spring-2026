...
## Ray Initialization — `ray.init()` {.smaller}
...
* Transform (note that you need to restrict `pandas` to a version < 3.0)
```{{python}}
import pandas as pd
...
## Model Evaluation & Batch Prediction {.smaller}
