...
Lab 10's four-stage DAG progression teaches the full ETL-to-cloud arc:
...
Lab 10's `gen_sql_query` task generates this INSERT:
...
In Lab 10, Airflow runs on a single EC2 instance using **`airflow standalone`** — a development-mode process that combines all components:
...
::: {.callout-note}
`airflow standalone` is for **development only** — single-node, SQLite, no HA. Production uses separate scheduler, API server, and Celery or Kubernetes executor with Postgres.
:::
...
- Lab 05 (Athena): GitHub Classroom link in Blackboard
- Lab 10 (Airflow): GitHub Classroom link in Blackboard
...
::: {.callout-important}
## Midterm 2 starts now

* Duration: **60 minutes**
* Access the problem set from GitHub Classroom (link posted in Blackboard and Slack)
* Work on your **EC2 cluster** with Spark — the cluster you started at the beginning of class
* **Open notes, open internet** — no AI tools (ChatGPT, Claude, Copilot, etc.)
* Submit your completed `.ipynb` via GitHub Classroom before time is called
* If your cluster is not running, start it now — you will not receive extra time
:::
